...
(defun my-paren-highlighting-hidden ()
  ...
  ;; Override parinfer faces specifically
  (when (bound-and-true-p parinfer-rust-mode)
    (let ((bg-color (my-paren-get-background-color)))
      ;; Override common parinfer faces
      ...
      (push (face-remap-add-relative 'parinfer-rust-dim-parens
                                     :foreground bg-color)
            my-paren-face-remaps)
      (push (face-remap-add-relative 'show-paren-match
                                     :foreground bg-color :background bg-color)
            my-paren-face-remaps)
      (push (face-remap-add-relative 'show-paren-mismatch
                                     :foreground bg-color :background bg-color)
            my-paren-face-remaps)))

  ;; Use high priority font-lock keywords to override everything else
  (font-lock-add-keywords
   nil
   '(("(\\|)\\|\\[\\|\\]\\|{\\|}" 0 'my-paren-hidden-face prepend))
   'prepend)
